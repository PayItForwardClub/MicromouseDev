...
void param_Set(E_PARAM_ID param_id, uint8_t *au8_value)
{
	...
	ui32_paramStartAddress -= ui16_param_length[0];
...
void param_Reset(void)
{
	...
	{
		for (j = 0; j < ui16_param_length[i]; j++)
		{
			EEPROMWrite((uint32_t *)&reset_value, ui32_paramStartAddress >> 2, 1);
			ui32_paramStartAddress += 4;
		}
	}
}